...
it('map-get-strict: valid map and string key returns value', () => {
...
it('map-get-strict: rejects non-map first argument', () => {
  const scss = createTestScss('$result: fn.map-get-strict("not-a-map", "key");');
  const { error } = compileScss(scss);
  expect(error, 'Expected Sass error but got none').toBeTruthy();
  expect(error).toMatch(/map-get-strict\(\): '\$map' must be a Sass map, got string./);
});

it('map-get-strict: non-string key throws error', () => {
  const scss = createTestScss('$map: (key1: "value1"); $result: fn.map-get-strict($map, 123);');
  const { error } = compileScss(scss);
  expect(error, 'Expected Sass error but got none').toBeTruthy();
  expect(error).toMatch(/map-get-strict\(\): token key must be a string, got number./);
});

it('map-get-strict: missing key throws error with available keys', () => {
  const scss = createTestScss('$result: fn.map-get-strict((key1: "value1"), "bogus");');
  const { error } = compileScss(scss);
  expect(error, 'Expected Sass error but got none').toBeTruthy();
  expect(error).toMatch(
    /map-get-strict\(\): Key "bogus" not found in \$map\. Available keys: key1/
  );
});

// Test shadow-border function
it('shadow-border: accepts valid states', () => {
...
it('shadow-border: rejects invalid state', () => {
  const scss = createTestScss('$result: fn.shadow-border(invalid);');
  const { error } = compileScss(scss);
  expect(error, 'Expected Sass error but got none').toBeTruthy();
  expect(error).toMatch(/shadow-border\(\): 'invalid' must be 'default' or 'hover', got invalid./);
});

// Test shadow-border mixin
it('shadow-border mixin: accepts valid states and rejects invalid ones', () => {
...
it('token-get: validates inputs correctly', () => {
  // Valid inputs
  ...
  expect(error, 'Expected Sass error but got none').toBeTruthy();
  expect(error).toMatch(/Invalid argument: '\$type' must be a string, got number/);
  ...
  for (const badType of ['false', "''"]) {
    ...
    expect(error, `Expected Sass error for $type ${badType} but got none`).toBeTruthy();
  }
...
it('token accessors: spacing/radius/elevation/breakpoint reject unknown keys', () => {
  const validCases = [
    'fn.spacing("2")',
    'fn.radius("sm")',
    'fn.elevation("2")',
  ...
  for (const [call, regex] of invalidCases) {
    const { error } = compileScss(createTestScss(`$result: ${call};`));
    expect(error, `Expected Sass error for ${call} but got none`).toBeTruthy();
  ...
  expect(customMap.error).toBeNull();
  expect(customMap.css).toMatch(/width: 100px/);
  ...
  expect(customMapBad.error, 'Expected Sass error but got none').toBeTruthy();
  expect(customMapBad.error).toMatch(/Key "bogus" not found in \$map\./);
  ...
  expect(emptyMap.error, 'Expected Sass error but got none').toBeTruthy();
  expect(emptyMap.error).toMatch(
    /breakpoint\(\): '\$map' must be a non-empty map, got an empty one/
  );
});

// Test button-variant mixin
it('button-variant: validates all parameters', () => {
  // Valid usage
  ...
  expect(error, 'Expected Sass error but got none').toBeTruthy();
  expect(error).toMatch(/Invalid argument: '\$base-css-var' must be a string, got number/);
  ...
  expect(error, 'Expected Sass error but got none').toBeTruthy();
  expect(error).toMatch(/button-variant\(\): 'border-alpha' must be between 0 and 1, got 1\.5/);
...
it('truncate-lines: validates line count', () => {
  // Valid value
  ...
  expect(error, 'Expected Sass error but got none').toBeTruthy();
  expect(error).toMatch(/Invalid argument: '\$lines' must be greater than or equal to 1, got 0./);

  const scssNegative = createTestScss('@include m.truncate-lines(-1);');
  ({ error } = compileScss(scssNegative));
  expect(error, 'Expected Sass error but got none').toBeTruthy();
  expect(error).toMatch(/Invalid argument: '\$lines' must be greater than or equal to 1, got -1./);

  const scssFraction = createTestScss('@include m.truncate-lines(1.5);');
  ({ error } = compileScss(scssFraction));
  expect(error, 'Expected Sass error but got none').toBeTruthy();
  expect(error).toMatch(/Invalid argument: '\$lines' must be an integer, got 1\.5/);
});

// Test aspect-ratio mixin
it('aspect-ratio: validates dimensions', () => {
  // Valid values
  ...
  expect(error, 'Expected Sass error but got none').toBeTruthy();
  expect(error).toMatch(/Invalid argument: '\$width' must be greater than 0, got -2./);

  const scssZeroWidth = createTestScss('@include m.aspect-ratio(0, 3);');
  ({ error } = compileScss(scssZeroWidth));
  expect(error, 'Expected Sass error but got none').toBeTruthy();
  expect(error).toMatch(/Invalid argument: '\$width' must be greater than 0, got 0./);

  const scssWithUnit = createTestScss('@include m.aspect-ratio(2px, 3);');
  ({ error } = compileScss(scssWithUnit));
  expect(error, 'Expected Sass error but got none').toBeTruthy();
  expect(error).toMatch(/Invalid argument: '\$width' must be a unitless number, got 2px/);
});

// Test elevation-shadow mixin
it('elevation-shadow: validates level and color', () => {
  // Valid level
  ...
  expect(error, 'Expected Sass error but got none').toBeTruthy();
  expect(error).toMatch(/elevation-shadow\(\): 'invalid-level' is not a valid elevation token\./);
  ...
  expect(error, 'Expected Sass error but got none').toBeTruthy();
  expect(error).toMatch(/elevation-shadow\(\): '123' is not a valid elevation token\./);

  // Invalid color type
  const scssBadColor = createTestScss('@include m.elevation-shadow("2", 123);');
  ({ error } = compileScss(scssBadColor));
  expect(error, 'Expected Sass error but got none').toBeTruthy();
  expect(error).toMatch(/Invalid argument: '\$color' must be a string, got number/);
  ...
  expect(error, 'Expected Sass error but got none').toBeTruthy();
  expect(error).toMatch(/Invalid argument: '\$color' must be a non-empty string/);
});

// Test elevation mixin
it('elevation mixin: rejects unknown tokens', () => {
  // Valid level
  ...
  ({ error } = compileScss(scssBadLevel));
  expect(error, 'Expected Sass error but got none').toBeTruthy();
  expect(error).toMatch(/Key "bogus" not found in \$elevation/);
});

// Test radius mixin and radius-corners
it('radius mixin: rejects unknown tokens', () => {
  // Valid token
  ...
  expect(css).toMatch(/border-radius: 0\.25rem/);

  // Invalid token
  const scssBad = createTestScss('@include m.radius("bogus");');
  ({ css, error } = compileScss(scssBad));
  expect(error, 'Expected Sass error but got none').toBeTruthy();
  expect(error).toMatch(/radius\(\): 'bogus' is not a valid radius token\./);
});

it('radius-corners: rejects unknown tokens', () => {
  // Valid tokens
  ...
  expect(error, 'Expected Sass error but got none').toBeTruthy();
  expect(error).toMatch(/Key "bogus" not found in \$radius/);
});

// Test icon-padding-adjust mixin
it('icon-padding-adjust: validates parameters', () => {
  // Valid values
  ...
  expect(error, 'Expected Sass error but got none').toBeTruthy();
  expect(error).toMatch(/Invalid argument: '\$side' must be one of start, end, got middle/);
...
it('concentric-radius: validates padding', () => {
  ...
  ({ error } = compileScss(scssNegative));
  expect(error, 'Expected Sass error but got none').toBeTruthy();
  expect(error).toMatch(/Invalid argument: '\$padding' must be greater than or equal to 0, got -5/);
...
it('font-face: validates all parameters', () => {
  // Valid parameters
  ...
  expect(error, 'Expected Sass error but got none').toBeTruthy();
  expect(error).toMatch(/Invalid argument: '\$name' must be a non-empty string/);

  // Empty path
  const scssEmptyPath = createTestScss('@include m.font-face("TestFont", "", 400);');
  ({ error } = compileScss(scssEmptyPath));
  expect(error, 'Expected Sass error but got none').toBeTruthy();
  expect(error).toMatch(/Invalid argument: '\$path' must be a non-empty string/);

  // Invalid weight
  const scssBadWeight = createTestScss('@include m.font-face("TestFont", "/path/to/font", -1);');
  ({ error } = compileScss(scssBadWeight));
  expect(error, 'Expected Sass error but got none').toBeTruthy();
  expect(error).toMatch(/Invalid argument: '\$weight' must be greater than or equal to 1, got -1/);
  ...
  expect(error, 'Expected Sass error but got none').toBeTruthy();
  expect(error).toMatch(/Invalid argument: '\$style' must be a string, got number/);
});

// Test transition mixin
it('transition: validates parameters', () => {
  // Valid parameters
  ...
  expect(error, 'Expected Sass error but got none').toBeTruthy();
  expect(error).toMatch(/Invalid argument: '\$property' must be a string, got number/);
  ...
  expect(error, 'Expected Sass error but got none').toBeTruthy();
  expect(error).toMatch(/transition\(\): '\$duration' must be a non-negative time value, got fast/);

  // Invalid duration (negative)
  const scssBadDur = createTestScss('@include m.transition(background-color, -0.1s);');
  ({ error } = compileScss(scssBadDur));
  expect(error, 'Expected Sass error but got none').toBeTruthy();
  expect(error).toMatch(/transition\(\): '\$duration' must be a non-negative time value/);
  ...
  expect(error, 'Expected Sass error but got none').toBeTruthy();
  expect(error).toMatch(/Invalid argument: '\$easing' must be a string, got number/);
});

// Test respond-to-all mixin
it('respond-to-all: validates breakpoint map', () => {
  // Valid map
  const scss = createTestScss('@include m.respond-to-all((sm: 576px, md: 768px)) { color: red; }');
  let { css, error } = compileScss(scss);
  expect(error).toBeNull();
  // Check that media queries were generated
  expect(css).toMatch(/@media screen and \(min-width: 576px\)/);
  expect(css).toMatch(/@media screen and \(min-width: 768px\)/);

  // Invalid (not a map)
  const scssNotMap = createTestScss('@include m.respond-to-all("not-a-map") { color: red; }');
  ({ css, error } = compileScss(scssNotMap));
  expect(error, 'Expected Sass error but got none').toBeTruthy();
  expect(error).toMatch(/Invalid argument: '\$breakpoints' must be a map, got string/);
});

// Test respond-to / respond-below mixins
it('respond-to / respond-below: reject unknown breakpoints', () => {
  // Valid breakpoint
  ...
  expect(css).toMatch(/@media screen and \(min-width: 990px\)/);

  // Invalid breakpoint
  const scssBad = createTestScss('@include m.respond-to("bogus") { color: red; }');
  ({ css, error } = compileScss(scssBad));
  expect(error, 'Expected Sass error but got none').toBeTruthy();
  expect(error).toMatch(/Key "bogus" not found in \$breakpoints/);
...
it('spacing-utility: validates parameters', () => {
  // Valid parameters
  ...
  expect(error, 'Expected Sass error but got none').toBeTruthy();
  expect(error).toMatch(/Invalid argument: '\$prefix' must be a non-empty string/);
...
it('hover-active-pressed: validates prefix parameter', () => {
  ...
  expect(error, 'Expected Sass error but got none').toBeTruthy();
  expect(error).toMatch(/Invalid argument: '\$prefix' must be a string, got number/);
...
it('active-pressed: validates prefix parameter', () => {
  // Valid prefix
